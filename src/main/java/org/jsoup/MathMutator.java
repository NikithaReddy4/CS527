...
//> >=
//>= >
public class MathMutator extends VoidVisitorAdapter<String>
{
    ...
    public MathMutator(CompilationUnit originalCu) {
        ...
        if (!mutated) {
            ...
            for (BinaryExpr binaryExpr : exprNodeList) {
                ...
                if (binaryExpr.getOperator() == BinaryExpr.Operator.PLUS) {
                    ...
                    mutants.add(mutant);
                    mutantexp_withmutants.put(mutant, "The expression " + before_Expr.toString() + " changed to " + after_Expr.toString() + " after mutation.");
                }else if (binaryExpr.getOperator() == BinaryExpr.Operator.BINARY_AND) {
                    before_Expr = binaryExpr;
                    ...
                    mutants.add(mutant);
                    mutantexp_withmutants.put(mutant, "The expression " + before_Expr.toString() + " changed to " + after_Expr.toString() + " after mutation.");
                }else if (binaryExpr.getOperator() == BinaryExpr.Operator.XOR) {
                    before_Expr = binaryExpr;
                    mutantExpr.setOperator(BinaryExpr.Operator.BINARY_AND);
                    after_Expr = mutantExpr;
                    mutants.add(mutant);
                    mutantexp_withmutants.put(mutant, "The expression " + before_Expr.toString() + " changed to " + after_Expr.toString() + " after mutation.");
                }else if (binaryExpr.getOperator() == BinaryExpr.Operator.LEFT_SHIFT) {
                    before_Expr = binaryExpr;
                    mutantExpr.setOperator(BinaryExpr.Operator.SIGNED_RIGHT_SHIFT);
                    after_Expr = mutantExpr;
                    mutants.add(mutant);
                    mutantexp_withmutants.put(mutant, "The expression " + before_Expr.toString() + " changed to " + after_Expr.toString() + " after mutation.");
                }else if (binaryExpr.getOperator() == BinaryExpr.Operator.SIGNED_RIGHT_SHIFT) {
                    before_Expr = binaryExpr;
                    mutantExpr.setOperator(BinaryExpr.Operator.LEFT_SHIFT);
                    after_Expr = mutantExpr;
                    mutants.add(mutant);
                    mutantexp_withmutants.put(mutant, "The expression " + before_Expr.toString() + " changed to " + after_Expr.toString() + " after mutation.");
                }else if (binaryExpr.getOperator() == BinaryExpr.Operator.UNSIGNED_RIGHT_SHIFT) {
                    before_Expr = binaryExpr;
                    mutantExpr.setOperator(BinaryExpr.Operator.LEFT_SHIFT);
                    after_Expr = mutantExpr;
                    mutants.add(mutant);
